...
function buildNoCacheHtaccess(publishId: string) {
  // Aruba caches bare directory URLs (/menu-dinner/) aggressively.
  // A 302 to index.html?v=<publishId> forces Safari + aruba-proxy to fetch fresh HTML.
...
<IfModule mod_rewrite.c>
  ...
  RewriteRule ^$ index.html?v=${publishId} [R=302,L,QSA]
  # Stale/missing version query → current publish id
  RewriteCond %{QUERY_STRING} !(^|&)v=${publishId}(&|$)
  RewriteRule ^index\\.html$ index.html?v=${publishId} [R=302,L]
  RewriteCond %{QUERY_STRING} !(^|&)v=${publishId}(&|$)
  RewriteRule ^en\\.html$ en.html?v=${publishId} [R=302,L]
</IfModule>
`;
}
