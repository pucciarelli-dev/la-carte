...
(function () {
  ...
  (function canonicalPhpUrl() {
    var path = location.pathname
      // Bad [R] redirects on Aruba sometimes expose the physical docroot
      .replace(/^\\/web\\/htdocs\\/[^/]+\\/home(?=\\/|$)/, "");
    ...
    var target =
      file === "en.html" || file === "en" ? "en.php" :
      file === "en.php" ? "en.php" :
      "index.php";
    if (file === "index.php" || file === "en.php") {
      if (path !== location.pathname) location.replace(base + file);
      return;
    }
...
function buildRedirectHtml(targetFile: string, publishId: string) {
...
<body>
<p><a href="${target}">Apri il menu</a></p>
</body>
</html>
`;
}
...
function buildNoCacheHtaccess(publishPath: string, publishId: string) {
  const path = publishPath.replace(/^\/+|\/+$/g, "");
  const base = `/${path}`;
  return `# Generated by La Carte — publish ${publishId}
# Never DirectoryIndex the PHP menu: Aruba caches that 200 HTML on /menu-*/.
# index.html is only a tiny redirect stub; rewrite also forces HTTPS → index.php.
...
<IfModule mod_rewrite.c>
  RewriteEngine On
  RewriteBase ${base}/
  # https:// host — Aruba TLS terminates at proxy, so %{HTTPS} is often off
  RewriteRule ^$ https://%{HTTP_HOST}${base}/index.php [R=302,L]
  RewriteRule ^menu\\.html$ https://%{HTTP_HOST}${base}/index.php [R=302,L]
  RewriteRule ^index\\.html$ https://%{HTTP_HOST}${base}/index.php [R=302,L]
  RewriteRule ^en\\.html$ https://%{HTTP_HOST}${base}/en.php [R=302,L]
  RewriteRule ^en$ https://%{HTTP_HOST}${base}/en.php [R=302,L]
</IfModule>
`;
}
...
export async function generateStaticMenuFiles(input: {
  ...
  return [
    ...
    },
    {
      ...
      content: Buffer.from(buildRedirectHtml("index.php", publishId), "utf8"),
