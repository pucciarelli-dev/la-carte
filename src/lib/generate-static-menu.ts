...
(function () {
  ...
  document.querySelectorAll('[aria-label="Lingua menu"] a').forEach(function (a) {
    ...
    // Clean stable URLs — PHP entrypoints always serve the latest HTML
    if (label === "IT") a.setAttribute("href", "./");
...
function buildServePhp(contentFile: string) {
  // Keeps the browser on a clean URL (/menu-wine/) while always reading
  // the latest published HTML from disk (bypasses Safari/Aruba HTML cache).
  return `<?php
...
<IfModule mod_rewrite.c>
  RewriteEngine On
  # Old bookmarked menu.html?v=&t= → clean directory URL
  RewriteCond %{QUERY_STRING} .
  RewriteRule ^menu\\.html$ ${webBase}/? [R=302,L]
  RewriteCond %{QUERY_STRING} .
  RewriteRule ^en\\.html$ ${webBase}/en.php [R=302,L]
  # Serve content files through PHP so reload is always fresh
...
export async function generateStaticMenuFiles(input: {
  ...
  return [
    ...
    {
      // Fallback if PHP is unavailable
